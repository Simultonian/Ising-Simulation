...
def test_match_brute_hue():
    ...
    sorted_pairs = list(
        sorted(zip(hamiltonian.paulis, hamiltonian.coeffs), key=lambda x: abs(x[1]), reverse=True)
    )
...
def test_match_brute_hue_delta_cutoff():
    ...
    brute = brute_first_order(hamiltonian.sparse_repr, time, error)
    hue = hue_first_order(sorted_pairs, time=time, error=0.1, delta=delta, cutoff_count=25*l)
