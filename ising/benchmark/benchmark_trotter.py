...
from ising.simulation.trotter import GroupedLieCircuit
...
def get_trotter_results(qubit, time, h_val, rho_init, observable, r_range):
    h_para = Parameter("h")
    ham = parametrized_ising(qubit, h_para)
    # This 0.1 will not be used to calculate `r`.
    circuit_manager = GroupedLieCircuit(ham, h_para, 0.1)
    circuit_manager.subsitute_h(h_val)
    circuit_manager.construct_parametrized_circuit()
