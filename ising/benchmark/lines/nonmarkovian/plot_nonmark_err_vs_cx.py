...
def test_main():
    file_name = f"data/benchmark/line/nonmarkovian/err_{QUBIT_COUNT}.json"
    ...
    for ind, (method, time_gate) in enumerate(results.items()):
        x_values, y_values = [], []
        for err_str, res_str in time_gate.items():
            error, res = float(err_str), float(res_str)
    ...
    file_name = f"plots/benchmark/lines/nonmarkovian/nolabel_error_{QUBIT_COUNT}.png"
