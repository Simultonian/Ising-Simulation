...
def interaction_hamiltonian_sparse(QUBIT_COUNT, gamma):
    """
...
def save_interaction_hams(qubit_count):
    ham_ints_sparse = interaction_hamiltonian_sparse(qubit_count, gamma=1)
    result = {}
...
def load_interaction_hams(qubit_count, gamma):
    with open(f"data/lindbladian/hamiltonian/size_{qubit_count}.json", "r") as file:
        result = json.load(file)

    ham_ints = []
    for _, desc in result.items():
        ham_ints.append(np.sqrt(gamma) * SparsePauliOp(desc["paulis"], desc["coeffs"]))
