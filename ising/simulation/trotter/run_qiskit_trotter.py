...
import json
import numpy as np
from ising.simulation.trotter import QiskitTrotter
...
def run_qiskit_trotter(paras):
    ...
    for num_qubit in range(paras["start_qubit"], paras["end_qubit"] + 1):
        ...
        for h in h_values:
            ...
            simulator = QiskitTrotter(assigned_ham, exact_simulator.observable)
...
def main():
    ...
    results = run_qiskit_trotter(parameters)

    file_name = f"data/output/ising_qiskit_trotter_{parameters['start_qubit']}_to_{parameters['end_qubit']}.json"
